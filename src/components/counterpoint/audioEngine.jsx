...
export async function playNoteWithCustomInstrument(pitch, duration, volume, customConfig, pitchBend = 0, articulation = 'normal', tempo = 80) {
  ...
  // Electric guitar velocity-based power chords FIRST (before articulations)
  if (customConfig.name === 'Electric Guitar') {
    if (volume >= 0.95) {
      // Maximum velocity - aggressive power chord with articulation applied
      const chordPitches = getPowerChordPitches(pitch);
      
      if (articulation && articulation !== 'normal' && articulation !== 'legato') {
        // Apply tremolo to each chord note
        if (articulation.startsWith('tremolo-')) {
          const speeds = { 'tremolo-slow': 2, 'tremolo-medium': 1, 'tremolo-fast': 0.5, 'tremolo-ultra': 0.25 };
          const volumeMults = { 'tremolo-slow': 0.9, 'tremolo-medium': 0.85, 'tremolo-fast': 0.8, 'tremolo-ultra': 0.75 };
          const tremoloSpeed = sixteenthNoteDuration * speeds[articulation];
          const numNotes = Math.floor(duration / tremoloSpeed);
          
          for (let i = 0; i < numNotes; i++) {
            setTimeout(() => {
              chordPitches.forEach((chordPitch, j) => {
                playSingleCustomNote(chordPitch, tremoloSpeed * 0.9, volume * volumeMults[articulation] * (1 - j * 0.1), customConfig, pitchBend);
              });
            }, i * tremoloSpeed * 1000);
          }
          return null;
  ...
  // Handle articulations for single notes
