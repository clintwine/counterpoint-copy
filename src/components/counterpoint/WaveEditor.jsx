    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        ...
        <div className="w-32 flex-shrink-0 space-y-1">
          <Label className="text-white/70 text-[10px] uppercase tracking-wider">Presets</Label>
          <div className="bg-slate-700/50 rounded-lg p-2 space-y-1 max-h-[120px] overflow-y-auto">
            {customInstruments.length === 0 ? (
              <p className="text-white/40 text-[10px] text-center py-2">No presets</p>
            ) : (
              customInstruments.map((inst, i) => (
                <Button
                  ...
                  className={`w-full h-6 text-[10px] justify-start px-2 ${editingIndex === i ? 'bg-amber-500/20 text-amber-400' : 'text-white/70 hover:text-white'}`}
                >
                  {inst.name}
                </Button>
              ))
