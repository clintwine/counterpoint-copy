  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < numBars; i++) {
        ...
        if (barHeight < rect.height * 0.3) { gradient.addColorStop(0, '#00D4FF'); gradient.addColorStop(1, '#0088FF'); }
        else if (barHeight < rect.height * 0.6) { gradient.addColorStop(0, '#00FF88'); gradient.addColorStop(0.5, '#88FF00'); gradient.addColorStop(1, '#00D4FF'); }
        else if (barHeight < rect.height * 0.8) { gradient.addColorStop(0, '#FFCC00'); gradient.addColorStop(0.5, '#00FF88'); gradient.addColorStop(1, '#00D4FF'); }
        else { gradient.addColorStop(0, '#FF3333'); gradient.addColorStop(0.3, '#FFAA00'); gradient.addColorStop(0.6, '#00FF88'); gradient.addColorStop(1, '#00D4FF'); }
        ...
        ctx.closePath();
        ctx.fill();
        if (value > 180) { ctx.shadowBlur = 8; ctx.shadowColor = barHeight > rect.height * 0.8 ? '#FF3333' : '#00FF88'; ctx.fill(); ctx.shadowBlur = 0; }
      }
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.05)';
      ctx.lineWidth = 1;
      for (let i = 0; i < 4; i++) { const y = (rect.height / 4) * i; ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(rect.width, y); ctx.stroke(); }
      const freqMarkers = [{ freq: '20Hz', pos: 0.02 }, { freq: '100Hz', pos: 0.15 }, { freq: '500Hz', pos: 0.35 }, { freq: '1kHz', pos: 0.5 }, { freq: '5kHz', pos: 0.75 }, { freq: '10kHz', pos: 0.9 }];
    ...
    draw();
    return () => { if (animRef) cancelAnimationFrame(animRef); };
  }, [canvasRef.current]);
}